...
import { getCountCartItems } from "./get-count-cart-items";
...
describe("getCountCartItems", () => {
  ...
  it("returns the summed quantity from the first row", async () => {
    ...
    const result = await getCountCartItems(MOCK_CART_ID);
  ...
  it("applies a where clause when userId is provided", async () => {
    ...
    const result = await getCountCartItems(MOCK_CART_ID, MOCK_USER_ID);
  ...
  it("propagates database errors", async () => {
    ...
    await expect(getCountCartItems(MOCK_CART_ID)).rejects.toThrow(
      "aggregation failed",
    );
  });
});
